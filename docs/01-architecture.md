...
**Обработка синхронная, без очереди.** Изначально планировалась BullMQ, но при реализации от неё отказались: сценарий отчёта — три-четыре запроса к БД, единицы миллисекунд при таймауте Telegram в 60 секунд. Очередь добавила бы воркер, сериализацию задач и отдельный мониторинг, не решая ни одной существующей проблемы. Дубликаты от ретраев закрыты идемпотентностью (см. ниже), а она нужна и при очереди тоже — ретрай положил бы в неё вторую задачу. Вернуться к очереди осмысленно, если в обработку добавится что-то долгое (распознавание фото, вызовы внешних сервисов).
