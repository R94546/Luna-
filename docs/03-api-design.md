...
`laborCost: null` — важная деталь: себестоимость работы берётся из **реальных расценок**, по которым платится зарплата. Иначе калькулятор начнёт врать в тот день, когда расценки поднимут. Берутся только действующие сегодня: у расценки есть срок, и прошлогодняя не должна занижать себестоимость. Если на операцию заведены и общая расценка, и персональная для сотрудника, в расчёт идёт одна — самая свежая, иначе работа посчиталась бы дважды.

Строка расхода — это либо `materialId` из справочника, либо разовый материал с `name`, `unit` и `unitPrice`: половина мелочи покупается один раз под конкретную модель, и заводить ради неё карточку никто не станет. **Для материала из справочника цена всегда берётся оттуда**, даже если в запросе прислали другую: посчитать себестоимость по цене, которой не существует, быть не должно.

`marginPercent` — наценка на себестоимость, а не доля в выручке: в цехе считают «плюс тридцать процентов», и цифра означает именно это.

### `POST /costing/:id/apply`

Записывает `totalCost` в `products.costPrice`. Отдельным действием, а не при сохранении: расчёты часто делают «на посмотреть», а от `costPrice` считается прибыль по каждой продаже.

Уже проведённые продажи не меняются — у них своя себестоимость, снятая в момент продажи. Иначе прошлая прибыль поехала бы задним числом.

Прежний применённый расчёт по этой модели теряет флаг `isApplied`: он означает «эта цифра сейчас стоит в карточке», и двух таких быть не может. Расчёт без `productId` применить нельзя — `422 CALCULATION_WITHOUT_PRODUCT`.

В сохранённом расчёте название и цена материала лежат **снимком**: материал переименуют, подорожает или уйдёт из справочника, а расчёт обязан остаться читаемым.
